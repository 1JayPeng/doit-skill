...
**CodeGraph tools for review (pick what fits, don't call all of them):**
- `codegraph_context(task="<review focus>")` — focused context for changed area
- `codegraph_search(query="<symbol>")` — find specific symbols
- `codegraph_callers(symbol)` / `codegraph_callees(symbol)` — call graph
- `codegraph_impact(symbol)` — blast radius of changes
- `codegraph_node(symbol)` — full source for a symbol
- `codegraph_explore(query)` — multiple related symbols at once
- **Fallback:** `git diff` + manual `Read` of changed files.
...
**⚠️ 批量编辑铁律 — 防止重复清理循环：**
- **先搜全量，再一次性编辑。** 任何清理操作（删变量、改引用、去 import、改排除列表），先用 `ctx_search` 找出文件中**所有**位置，然后用单次 Edit 的 `replace_all` 一次性改完。
- **禁止分次清理同一类事物。** 不要"清理一处 → 检查 → 再清理一处"。一次找全 → 一次改完 → 验证完毕。
- **连续 2 次同类编辑 = 停止。** 如果刚对同一文件做了同类编辑（如两次都删 `context_cols`），说明第一次没搜全。改用 `ctx_search` 找全，然后批量处理。
...
1b. **Type-checker clean again after simplify** — `cargo check`, `tsc --noEmit`, `pyright`
2. `ctx_search(queries=[<old_functionality_keywords>])` — verify documentation matches new behavior
3. One final read of the modified files — do they still read correctly after simplifying?

**Fallback:** `git diff <files>` + manual `Read`.
