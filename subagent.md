...
**子代理通过 `general-purpose` agent type 获得完整工具权限。** 工具使用是省 token 的关键（tokensave 替代 Explore agent，context-mode 替代 Bash）。

| 工具 | 用途 | 省 token 效果 |
|------|------|--------------|
| tokensave | 代码图分析（符号搜索、调用关系、影响范围） | 替代 grep+Read，减少 60-80% 上下文读取 |
...
| AskUserQuestion | 交互式提问 | 非阻塞式用户决策 |
| Agent | 嵌套子代理 | 递归并行（慎用，token 成本高） |

**子代理可用所有 Claude Code 工具。** 通过 `subagent_type: "general-purpose"` 获得与主代理相同的工具集。工具链正确配置是子代理效率的关键 — 有 tokensave 的子代理比只有 Bash+Read 的子代理快 3-5 倍。
