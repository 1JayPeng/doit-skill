...
单次 `/doit` 调用可能无法完成整个工作流 — 规格验证、实现、测试和审查是一个漫长的过程。要从中断处恢复，在当前对话中再次输入 `/doit` 即可。工作流程从当前阶段继续（记录在 `.scratch/workflow-state.json` 中）。
...
| skill-creator | `claude install anthropics/skills/skill-creator` | 技能开发 |
