...
- 新增铁律："完整工作流不可跳过" — 每个 phase 必须按顺序完成，不允许跳过
- 新增铁律："Review + Simplify 不可跳过" — 未经审查的代码不能提交
- Review 检查清单：重复代码、安全漏洞(OWASP)、过度抽象、死代码、README 同步
- Simplify 检查清单：合并重复、删除死代码、压平抽象、减少行数
- 禁止行为：跳过 Review、跳过 Simplify、走过场审查、简化后跳过 E2E
- CLAUDE.md 注入：Workflow Rules 节传播到每个项目
- 铁律总数：3 → 6

**2026-06-04** — 强化后台执行铁律：
- 每次 Bash 前强制决策门控 — 估计运行时间、选择级别、设置完成信号
- 明确禁止行为 — 禁止空等、禁止无轮询的后台任务、禁止忘记后台任务
- 后台任务检查清单 — 每次启动后台任务后 4 项验证
- 强化 CLAUDE.md 注入 — 更强的后台任务规则（含强制轮询）写入项目 CLAUDE.md
- Per-phase 要求："所有 >10s 命令必须后台执行 + Monitor/ScheduleWakeup 轮询。无例外。"
