...
| 工具 | 作用 | 使用阶段 |
|------|------|---------|
| **TokenSave** | 代码图谱（符号、调用关系、依赖） | Phase 2-7 |
| **Context-Mode** | 会话上下文（命令输出索引、语义搜索） | Phase 3-7, 10 |
| **MemPalace** | 跨会话语义记忆（规格、决策、知识图谱） | Phase -1, 0, 1, 2, 3, 5, 8, 9.5, 10 |
| **RTK** | Token 优化（自动代理 Bash 命令，节省 60-90%） | 全部阶段 |
| **caveman** | 简洁模式（压缩上下文） | Phase 0+, 10 |
| **code-review** | 代码审查 | Phase 5 |
| **uv** | Python 包管理 | Phase 3 |
| **Tavily MCP** | 互联网搜索 | Phase 1 |

### 三层记忆架构

| 层级 | 工具 | 持久化范围 |
|------|------|-----------|
| 代码层 | TokenSave | 代码符号、调用关系、依赖图 |
| 会话层 | Context-Mode | 命令输出、语义搜索索引 |
| 跨会话层 | MemPalace | 规格、决策、实现笔记、知识图谱 |

三层协作 -> 上下文在会话间不丢失。任何一层不可用 -> 优雅降级。
