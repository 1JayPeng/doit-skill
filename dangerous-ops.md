...
- 如需删除，先通过 AskUserQuestion 确认
```

## Hook 配置（可选）

Phase -1 检测用户环境时，可以建议在 `.claude/settings.json` 中添加：
...
**Hook 不写入项目 settings.json。** Hook 配置通过 Phase -1 的环境检测写入用户全局 `~/.claude/settings.json`，或者仅作为建议告知用户。
