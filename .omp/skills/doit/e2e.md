...
**E2E 测试的核心不是工具调用，而是角色切换：agent 必须忘记自己是开发者，完全扮演客户。**

开发者看代码：函数、模块、内部状态。
客户看功能：输入、输出、结果、感受。

**Agent 必须完全切换到客户视角。** 这不是修辞，是行为规则。每一步操作前问自己：
- 我是一个刚拿到这个功能的客户，不会去读任何代码
- 我从程序入口开始，像第一次使用它一样操作
- 客户看到什么、听到什么、摸到什么，才是测试该验证的
- 如果客户说"这个功能不对"，bug 就记下来，而不是去查日志

**验证清单（客户视角自检）：**

```
- [ ] 这个测试的场景，客户在实际使用中会经历吗？
- [ ] 这个测试的断言，客户能理解是在验证什么吗？
- [ ] 这个测试只验证了用户可见的行为，没有触及内部函数？
- [ ] 如果把这个测试给不懂代码的客户看，他能看懂吗？
```
...
user = User("test")
user.login()
assert user.token  # 客户看不到这个

# ✅ agent 扮演客户测试
result = subprocess.run(["app", "login", "--user", "test"])
assert "token: abc123" in result.stdout  # 客户看到的就是这个
```

**与"从入口启动"的关系：** "从入口启动"是技术约束（必须经过 CLI / HTTP / GUI），"扮演客户"是视角约束（必须像用户一样思考）。两者叠加，才是完整的 E2E。
