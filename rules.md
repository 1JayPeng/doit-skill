...
新模式：调用 `[[USER:ask]]` 发出问题 -> 同时继续执行其他工作 -> 用户在后台回答 -> 收到答案后应用。

**`[[USER:ask]]` 是工具调用，不是文本输出。** 不要只是在响应中写下问题 — 必须调用实际的 `[[USER:ask]]` 工具（Claude Code: `AskUserQuestion`，OpenCode: `question`）。只有工具调用才会弹出用户可选的选项框。仅输出文本 = 用户看不到选项 = 无效提问。

**执行顺序（不可颠倒）：**
1. **先调用 `[[USER:ask]]` 工具** — 把问题发给用户，选项弹出
2. **同时继续执行** — 不等回答，做其他工作
3. **收到回答后** — 应用用户的选择
4. **用户始终未回答** — 用推荐默认值继续
...
- **绝不跳过 `[[USER:ask]]` 调用直接用默认值** — 跳过 = 用户看不到选项 = 用户以为被忽略了
